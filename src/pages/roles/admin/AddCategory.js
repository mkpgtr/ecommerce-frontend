...
const AddCategory = () => {
    ...
    const [categoryToEdit,setCategoryToEdit] = useState(null)
      // ! this means the category name just before the add category is clicked
          // ! in the modal popup (i.e. just before editing)
    ...
    // ! this gets triggered when the add category button in the modal is clicked
  ...
  return (
    ...
    <div className="row mb-3">
      ...
      <div className="col-sm-10 inputCustom">
        <input type="text" name ='name' className="form-control form-control-sm" onChange={handleChange} id="colFormLabelSm" placeholder="Please enter category name" />
      </div>
      
    </div>
    <div className="row mb-3">
    <div className="col-sm-10 inputCustom button">
      <button type="button" onClick = {()=>sendPostRequestToCategoryRouteInTheBackend()} className="form-control form-control-sm file" id="colFormLabelSm"> Add Category</button>
    </div>
...
<div class="row justify-content-around gap-4">
    {
        allCategories?.map((singleCategory)=>{
            return  <div key={singleCategory._id} onClick={()=>{
              setShowModal(true);
              setCategoryToEdit(singleCategory)
              setCategoryNameBeforeUpdate(singleCategory.name)
            }} style={{"cursor":"pointer"}} class="col-lg-2 col-md-4 border border-dark text-center">
            {singleCategory.name}
          </div>
